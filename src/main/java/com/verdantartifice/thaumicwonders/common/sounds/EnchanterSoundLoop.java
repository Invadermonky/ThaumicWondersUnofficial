...
public class EnchanterSoundLoop extends PositionedSound implements ITickableSound {
    protected TileEssentiaEnchanter enchanter;
    protected BlockPos position;

    public EnchanterSoundLoop(SoundEvent event, TileEssentiaEnchanter enchanter, float volume) {
    ...
    @Override
    public void update() {
        if (this.enchanter.isInvalid() || this.enchanter.getProgress() <= 0) {
            this.volume -= 0.05F;
        }
    }
